...
|---|---|---|---|
| `apply_h_kinema` | Lanczos / CFM4:2 (Magnus 系) の合成 matvec | Phase 1 | overwrite: `y = a·H_drv·v + b·H_p_diag·v` (cache-blocked) |
...
- **Phase 6 C2 (issue #63) で `wide::f64x4` 経由の SIMD 特化を導入済み**。
  `apply_h_kinema` の bit-flip pass の i ∈ {0,1,2} (stride 1/2/4 連続
  アクセス領域) を `simd_kernels::bitflip_iN` に dispatch する。SIMD inner
  kernel は `apply_h_kinema_serial` と `_rayon` の両 path から共通で
  ...
  側に集中して実装済み (§5.1.3) で `apply_h_kinema` 本体は touch せず.
  `apply_h_kinema` の DRAM bandwidth 改善は **issue #79 (Phase 6 D)** で
  group-fused 3-phase 形を試行したが Linux 本番 bench + perf 計測で
  **真の compute regression が確認され未採用** (詳細は §5.1.4).
- **dim 閾値による rayon dispatch (issue #68, follow-up)**: `apply_h_kinema`
...
- **既存 `apply_h_kinema` の cache-blocked 形は維持** (Lanczos hot path は
...
pub fn trotter_step(
    ...
    for i in 0..n {
        ...
        // (H_drv の負符号は θ では巻き取らない. apply_h_kinema の
...
3. **`apply_h_kinema` と同じ層に揃える**: Phase 6 の cache
...
- **C2.5 (#71, 実装済み)**: SIMD 特化 (i ∈ {0,1,2}) を `wide::f64x4` で導入.
  C2 で `apply_h_kinema` 側に作った `simd_kernels::bitflip_iN` と同じ
  ...
  - **chunk_size は動的計算** `(dim/(nth·4)).clamp(MIN, MAX)` + block 丸め
    + SIMD_BLOCK_MAX (= 8 Complex64) 丸め. `apply_h_kinema_rayon` /
...
|---|---|---|---|
| `apply_h_kinema` | 23.8 ms | **6.13× (64 threads)** | DRAM bandwidth 上限 (理論 64× の ~9.6%) |
...
kinema の `H_drv = -Σ h_x_i X_i` は **per-site で commuting** なので
...
pub(crate) fn apply_multi_qubit_gate_fused(
    ...
    // chunk_size を thread 数に応じた動的計算 (apply_h_kinema_rayon と同型)
...
kinema は `k = 4` を default とし `bench_block_fusion.py` で sweep で確認.
...
apply_h_kinema 経路 (cfm4_step / m2_midpoint_step 内) は既存実装で 1 回の
...
##### C. `apply_h_kinema` の chunk_size: 旧値維持

PR #78 初版で `RAYON_CHUNK_MAX` を `1 << 14` (= 256 KB) → `1 << 13` (= 128 KB)
に縮める変更を入れたが, Linux 本番 bench で `apply_h_kinema` の **N=18
...
- bench: `benchmarks/bench_block_fusion.py` で N ∈ {18, 20, 22} の per-step
  time を `trotter_step` / `apply_h_kinema` 両方計測.
...
- **実測 (Linux x86_64, cpu_count=64, RAYON_NUM_THREADS=64, BLAS=1, PR #78 v3)**:
  ...
  | N | `trotter_step` speedup | `apply_h_kinema` speedup |
  |---|---|---|
  | 18 | **1.55×** | 0.94× (誤差範囲) |
  | 20 | **4.01×** ✅ | 0.94× (誤差範囲) |
  | 22 | **2.93×** | 1.01× |
  ...
  `apply_h_kinema` は本 issue で touch せず ≈ 1.0× (regression なし).
  apply_h_kinema の DRAM bandwidth 改善は follow-up issue #79 (Phase 6 D)
...
- **IPC 0.56 → 0.94 (+68%)**: barrier 待ちで idle cycle が大量に発生して
  いた pre-C3 状態を解消. ただし `apply_h_kinema` の IPC 2.84 にはまだ
  遠く, trotter_step 経路は依然 L2 fill latency bound 寄り (avg L2 latency
  393 cycles は `apply_h_kinema` の 216 cycles の 1.8 倍).
...
- **連続 k qubit 限定 (任意 qubit 集合の fusion は対象外)**: kinema の
  ...
  汎用回路シミュレータの要件で, kinema では不要.
- **`apply_h_kinema` 側に gate fusion を入れない**: matvec は **sum of
  ...
  cache fusion (C1) が apply_h_kinema の最適解.
...
#### 5.1.4 Phase 6 D 実験アーカイブ — `apply_h_kinema` の DRAM bandwidth 改善試行と未採用 (issue #79, 2026-05-17)

issue #68 で `apply_h_kinema` が N=20 / 64 threads で **6.13× scaling
...
`apply_h_kinema_rayon` を 3 phase に分割し, 2^fused_k 個の連続 chunk を
...
`src/bin/perf_apply_h.rs` (純 Rust binary, `apply_h_kinema` を 500 回呼ぶ)
...
4. **Python bench (`bench_block_fusion.py`) の N=18 で観測した 0.53× 大幅
   ...
   `apply_h_kinema_py` が毎回 64 MB pyarray を alloc/copy する overhead が
...
- **`src/bin/perf_apply_h.rs`** + `_rust::bench_api::apply_h_kinema`
...
- **issue #79 を re-open しない理由**: 本 hardware (AMD EPYC 7713P) では
  apply_h_kinema の「compute 効率」自体が改善余地ない. 別 hardware
  または大規模 N で真の DRAM bound が観測されたら新 issue を立てる方が
  scope が明確.

##### Phase 6 follow-up: Python bench alloc noise 部分解消 (issue #85, 2026-05-17)

§5.1.4 で得た「Python bench (`bench_*.py`) は `apply_h_kinema_py` の
allocate-and-return 経路の alloc/copy が wall time の大半を占めるので
Rust 側 micro-optimization の検証には不適」という知見を受け, **`apply_h_kinema`
の Python 入口に in-place 版 `apply_h_kinema_into_py` を追加** した
(issue #85, `docs/design/07-rust-extension.md` §7.3.1).

issue #85 で移行した call site は以下:

- `python/kinema/eigenstates.py::_eigenstates_lanczos` (m=64 Krylov loop)
- `python/kinema/eigenstates.py::_eigenstates_exact` (dim 列ループ)
- `benchmarks/bench_block_fusion.py::_measure_apply_h_kinema`
- `benchmarks/bench_simd_scaling.py::_measure_apply_h`
- `benchmarks/bench_parallel_scaling.py::child_run` (apply_h_kinema セクション)
...
- **dynamic**: `(dim/(nth·4)).clamp(MIN, MAX)` + block 丸め + SIMD_BLOCK_MAX
  丸め. `apply_h_kinema_rayon` / `apply_multi_qubit_gate_fused_rayon`
