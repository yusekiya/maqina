...
kinema では `H(t) = A(s(t))·H_driver + B(s(t))·H_problem` の構造を持つ
...
`chebyshev_tol` は `krylov_tol` と同じ "Krylov 近似の許容誤差" 規約を流用
(issue #98 Phase 8 の意味再定義準拠; Phase B では新パラメータを導入しない
方針)。Public Python API では `QuantumAnnealer.krylov_tol` をそのまま渡せる。
...
resolution は facade 層 (`python/kinema/annealer.py` の
...
resolution は全て facade 層 (`python/kinema/annealer.py`) で行い,
