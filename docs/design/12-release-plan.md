...
- `Schedule` プリセット: `linear` / `from_callable` / `reverse` / `pause`
  ...
  時点で同梱)
- Rust 拡張: `apply_h`, `lanczos_propagate`, `m2_midpoint_step`,
...
- **L2 並列化 (C1, issue #62, 実装済み)**: matvec / Trotter primitives の
  bit-flip pass を rayon `par_chunks_mut` で並列化。`apply_h` と
...
- **SIMD (C2, issue #63, 実装済み)**: `wide` クレート (stable Rust + 自動
  target_feature 切替) で f64x4 (AVX2 / AVX-512 / NEON) を使い, `apply_h`
  ...
  `apply_h_serial` と `apply_h_rayon` の両 path から
  ...
  **両ビルドで bit-identical または rel < 1e-13** な数値結果を返す
  (`apply_h_simd_matches_scalar_fuzz_100iter` テスト, src/matvec.rs)。
...
- **cache block-fusion (C3, issue #64, 実装済み, PR #78 merged)**: 当時
  ...
  **本 issue では trotter_step 側を集中改善** (`apply_h` 側は follow-up #79
  ...
  - **C. chunk_size 戦略**: PR #78 初版で `RAYON_CHUNK_MAX` を `1<<14` →
    ...
    旧値に復活. A の fused 経路でも `apply_h_rayon` と同じ
  ...
  **実測 (Linux x86_64, cpu_count=64, BLAS=1)**: `trotter_step` の per-step
  ...
  N=22 で 2.93× の改善. `apply_h` は本 issue で touch せず
  ≈ 1.0× (regression なし).
- **C2.5 (issue #71, 実装済み)**: `apply_single_mode_axis_i` の SIMD 特化
  (i ∈ {0,1,2}). C2 で `apply_h` 側のみ SIMD 化したため,
...
- 実装: `apply_h_drv` / `apply_h_p_diag` primitive を `src/matvec.rs` に追加,
  ...
  既存 `apply_h` の cache-blocked 形は維持 (hot path 触らない).
...
には程遠い) と判明。`apply_h` は #62 で rayon 並列化済だが,
...
- Rust 内部関数: `apply_h_kryanneal*` → `apply_h*` 全 path
