...
- [x] **Step 2**: `benchmarks/bench_m_eff_adiabatic.py` を β_m / err_lanczos
      / err_magnus 軸で拡張.
- [ ] **Bench acceptance** (#93 perf): Linux サーバーで
      `bench_qutip_large.py --scenarios long-T --n-values 8,10` を取り,
      CFM4 adaptive Richardson が QuTiP と少なくとも同等以上の Pareto position
      に来ること. step reject 数が default 設定で増加しないこと.

### Out of scope (Phase 7+ follow-up issue 候補)
