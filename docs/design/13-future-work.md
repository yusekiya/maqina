# §13. Future work (v0.11 までは対応しない範囲)

以下の項目は **v0.11 までのリリース計画には含めない** ことが確定済み
(Phase 計画は §12 参照)。リクエストが出てきた時点で v0.12+ として再評価する。

- **複数の driver 形 (XX 等の k-local X)**: v0.1〜v0.11 では driver を
...
- **Z_2 (global spin-flip) 対称性によるセクタ分割**: `Π = Π_i X_i` は
  ...
  になる。実装は「Phase 1-8 + A/B の汎用経路は対称性に依らず動く + ユーザが
  hint (`IsingProblem(..., symmetry='z2')` など) を渡したときに後段で
  fast path に切替える」構成が綺麗。利得は最大 2× で条件付きのため、
  汎用経路の安定化後 (= Phase B finalize 完了後 = v0.11 以降) に検討する。
