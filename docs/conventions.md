...
- Lint: `ruff` / 型: `ty`
- Rust MSRV: **1.88** (`Cargo.toml` の `package.rust-version` で pin)。
  SIMD カーネルの chunk 走査に使う `slice::as_chunks` /
  `as_chunks_mut` の安定化バージョン (issue #169)。clippy は
  `package.rust-version` を msrv 設定として読むため、MSRV 未満の機能を
  勧める lint は自動で抑制される。MSRV を引き上げるときは、引き上げの
  根拠となった std / 言語機能を本項に併記する。
