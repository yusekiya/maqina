...
- **Phase 6 C2 (issue #63) で `wide::f64x4` 経由の SIMD 特化を導入済み**。
  ...
  `apply_h_kryanneal` 自体の DRAM bandwidth 改善は **Phase 6 D (issue #79)**
  で高 i pass を group-fused 3-phase 形に書き換えた (§5.1.4).
- **Phase 6 D (issue #79) で高 i pass の group fusion を導入済み** (§5.1.4
  詳述). `apply_h_kryanneal_rayon` を「per-chunk diag + low-i」/
  「group-fused 高 i」/「per-chunk 残り高 i」の 3 phase に分け, 連続
  `fused_k` 個の高 i axes (mask ≥ chunk_size) の partner 参照を group
  (= 2^fused_k 個の連続 chunk) 内で完結させて L2 resident 化する. これに
  より DRAM 上の v traffic が `dim · (1 + h_naive)` 要素 (h_naive = n -
  log2(group_size)) に縮み, 従来の `dim · (1 + h_baseline)` から
  `(1 + h_baseline) / (1 + h_naive)` 倍改善. `fused_k = 0` (= 並列度確保
  優先のフォールバック条件) では従来の C1 経路にフォールバックする.
...
- **`apply_h_kryanneal` 側に tensor-product 形の gate fusion を入れない**:
  matvec は **sum of X_i + diag** で, 各 X_i は他 qubit に identity. これを
  tensor product `Π_j (I ⊗ ... ⊗ X_{i_j} ⊗ ... ⊗ I)` に畳んでも sparse な
  ままで `2^k × 2^k` dense matrix の利得が出ない. 一方で per-axis pass
  自体を **chunk-resident でなく group-resident に再構成** すれば DRAM
  traffic 自体を削減できる. これが Phase 6 D (§5.1.4) の発想で, C3 とは
  独立に effective に効く (C3 は trotter_step, D は apply_h_kryanneal の
  別 issue).
...
#### 5.1.4 Phase 6 D — `apply_h_kryanneal` の DRAM bandwidth 緩和 (高 i group-fused 3-phase)

Phase 6 C3 で `trotter_step` の DRAM 律速は解消されたが, **`apply_h_kryanneal`
自体は touch せず ≈ 1.0×** (#64 final bench, regression なし) のまま残った.
本 phase (issue #79) は `apply_h_kryanneal_rayon` の高 i pass (`mask >=
chunk_size`) を **group-fused 3-phase 形** に書き換えて DRAM v traffic を
削減する.

##### 構造

既存 C1 (`apply_h_kryanneal_rayon` Phase 6 C1 形) は chunk 単位の closure
で diag + 全 i pass を fuse する cache-blocked 形:

- 低 i (`mask < chunk_size`, `i < chunk_log`): partner `v[k ^ mask]` は同じ
  chunk 内 → L1 resident ✓
- 高 i (`mask >= chunk_size`, `i >= chunk_log`): partner `v[k ^ mask]` は
  **別 chunk** → 各高 i pass で chunk 1 個分の v を DRAM から再 load ✗

Phase D は 2^fused_k 個の連続 chunk を 1 つの **group** (super-chunk =
`group_size = chunk_size << fused_k`) にまとめ, group 内の高 i ペアを 1
thread に渡す (`par_chunks_mut(group_size)`). group 内 chunk index `c` から
axis `chunk_log + j` (j ∈ [0, fused_k)) の partner は `c ^ (1 << j)` で
group 内に閉じるため, group 全体が L2 resident なら高 i pass の partner 参照
は DRAM 再 load ゼロで済む.

closure 内は 3 phase:

1. **Phase 1 (per-chunk diag + low-i)**: 各 chunk について diag `y_chunk[li]
   = b_t · H_p[k] · v[k]` 上書き + 低 i (`i ∈ [0, chunk_log)`) の bit-flip
   pass を `y_chunk` L1 resident で完走. 既存 C1 と同型. SIMD i ∈ {0,1,2}
   は既存 `simd_kernels::bitflip_iN` を流用.
2. **Phase 2 (group-fused 高 i)**: 各 chunk c について fused_k 個の axes
   (`i ∈ [chunk_log, i_split)`, `i_split = chunk_log + fused_k`) を順次
   accumulate. 各 (c, j) ペアで `y_chunk[c] += coeff_j * v_partner[c]` を
   1 sweep で実行 (partner = group 内の別 chunk, L2 resident).
3. **Phase 3 (per-chunk 残り高 i, naive)**: `fused_k < n - chunk_log` の
   残り高 i axes (`i ∈ [i_split, n)`) は従来通り chunk ごとに naive な
   `v[k ^ mask]` 参照. partner は別 group の chunk → DRAM 経由 (改善対象外).

各 `y[k]` への accumulation 順序は **diag → i=0 → ... → i=n-1** で C1 / serial
と完全一致 (3 phase は i の昇順を分割するだけ). 従来テスト
`apply_h_kryanneal_rayon_matches_serial` / `..._determinism_8thread_100iter`
を **書き換え不要で bit-identical 維持**.

##### パラメータ選択

```text
chunk_size = (dim / (nth * 4)).clamp(RAYON_CHUNK_MIN, RAYON_CHUNK_MAX)
chunk_size = round_down_pow2(chunk_size).max(RAYON_CHUNK_MIN)
chunk_log = log2(chunk_size)
fused_k = min(
    n - chunk_log,                    # 高 i axes 本数
    MAX_FUSED_HIGH_K (= 6),           # L2 budget (group_size <= ~1 MB)
    log2(n_chunks_total) - log2_ceil(nth),  # 並列度 (n_groups >= nth)
)
group_size = chunk_size << fused_k
```

- `MAX_FUSED_HIGH_K = 6`: 2^6 = 64 chunks/group. Skylake server で L2 = 1
  MB/core, chunk_size = 1024 (16 KB Complex64) なら group_size = 64K = 1 MB.
- 並列度クランプ `log2(n_chunks_total / nth)`: thread 数 nth に対して
  n_groups = dim/group_size ≥ nth を保証. dim が小さく (`n_chunks_total <
  nth`) `fused_k = 0` に落ちる場合は Phase D 無効化 → 旧 C1 経路へ
  フォールバック (`apply_h_kryanneal_rayon_c1`).
- chunk_size を **pow2 に丸め下げ**: Phase D の XOR partner indexing は
  chunk_size = pow2 を仮定 (`mask = 1 << i` が chunk_size の倍数になる).
  RAYON_CHUNK_MIN = 64 が pow2 で下限なので丸め後も >= 64 が保たれる.

##### DRAM v traffic (per matvec call)

| 経路 | v traffic (elements) | 改善源 |
|---|---|---|
| 旧 C1 のみ | `dim · (1 + h_baseline)` | — |
| Phase D 適用 | `dim · (1 + h_naive)` | `h_naive = h_baseline - fused_k` |

`h_baseline = n - log2(chunk_size_baseline)`. 改善率 ≈ `(1 + h_baseline) /
(1 + h_naive)`. 具体例:

| N | dim | chunk_size | h_baseline | fused_k | h_naive | 改善 (理論) |
|---|---|---|---|---|---|---|
| 18 | 256K | 1024 | 8 | 2 | 6 | 9/7 ≈ 1.29× |
| 20 | 1M   | 4096 | 8 | 4 | 4 | 9/5 = 1.80× |
| 22 | 4M   | 16384 | 8 | 6 | 2 | 9/3 = 3.00× |

(64-thread Linux サーバー想定. N=22 で MAX_FUSED_HIGH_K のキャップに当たる.
nth が小さい環境 (例: macOS dev 8 thread) では `max_fused_by_par` ではなく
`MAX_FUSED_HIGH_K` か `n - chunk_log` でキャップされる.)

実測は bench (`benchmarks/bench_block_fusion.py --kernels apply_h_kryanneal`)
で確定. Phase 2 が cache miss を引かない前提が崩れたら h_naive 理論値より
小さい改善になる.

##### Acceptance と bench 運用

- bench: `benchmarks/bench_block_fusion.py --kernels apply_h_kryanneal` で
  N ∈ {18, 20, 22} の per-step time を計測 (script は C3 で整備済み).
- baseline: Phase 6 C2.5 完了時点 (`main` branch tip, PR merge 前).
- acceptance: N=20 で `apply_h_kryanneal` の per-step time が baseline の
  **>= 1.3×** 改善 (issue #79 当初目標).
- 副次目標: N ∈ {18, 22} で regression なし.
- 数値一致: `cargo test` (BLAS feature on/off) + `uv run pytest` で `rel <
  1e-13` 維持 (Phase D は accumulation 順序を C1 / serial と完全一致させて
  bit-identical 保証).

bench 実行は pre-merge bench cycle (`.claude/solve-overrides.md` 参照). Claude
は実装 + 修正 commit を PR branch に push, Linux 本番 bench は **ユーザーが
手動実行** して結果を共有する.

##### 設計判断の論拠

- **3 phase の順序を i 昇順に揃える**: 数値同一性のため. Phase 2 を Phase 1 の
  内側に入れて `y_chunk` ごとに完結させる "fully fused" 形も考えられるが,
  その場合 high i パスを c → j 順で回すと per-y[k] の accumulation 順序が
  `diag → low-i → high-i (j=0..fused_k-1)` で同じになるものの, group 内全
  chunk への同時アクセスを 1 closure に閉じ込めるため tight memory layout
  が要求され, 実装の複雑度が増す. **「`y_group` の chunks_mut を 3 回回す」
  方が読みやすく maintain しやすい** (各 phase で y_group の借用を分離).
  rayon barrier は変わらず 1 per call で, 演算量も同じ.
- **`MAX_FUSED_HIGH_K = 6` の上限**: 2^6 = 64 chunks per group. L2 = 1 MB/core
  (典型的 Skylake server) で group_size = 1 MB を超えると L2 spill して
  Phase 2 が cache miss に転落. macOS Apple Silicon (P core L2 = 12 MB
  共有) では余裕があるが, ARM 側は本番 bench 対象外 (Linux x86 のみ).
- **Phase 3 を残す**: 全 高 i を fuse すると `fused_k = n - chunk_log` で
  group_size = dim となり並列度が 1 になる. fused_k を `MAX_FUSED_HIGH_K`
  と `log2(n_chunks_total) - log2_ceil(nth)` でキャップして並列度を確保
  しつつ, fused 外の高 i は Phase 3 で従来通り naive 経路に流す.
- **代替案 (B/C/D) を本 phase では採用しない**: issue #79 本文に列挙された
  B (SIMD i≥3), C (prefetch), D (streaming store) は **A (group fusion)
  だけで acceptance pass しない場合の追加カード**. owner 分析コメント
  (issue #79 著者) で「A 単独 → A+streaming store」順で進める方針が確定.
  本 PR では A のみ実装, bench 結果次第で追加 phase を別 PR にする.
- **参考実装**: 状態ベクトル simulator の `X` gate group fusion は
  qsim (`lib/simulator_avx.h::ApplyGateLD`, `lib/fuser_mqubit.h`) や IBM
  AER の `qubitvector` で同様の発想がある. ただし qsim 系は任意 qubit 集合の
  fusion を `_pdep_u64` で実現する汎用形, kryanneal の場合は連続 qubit
  限定の単純化版で十分 (`H_drv = -Σ h_x_i X_i` は per-site で commuting,
  順序は physical i 固定).
...
- **SIMD (C2, issue #63, 実装済み)**: `wide` クレート (stable Rust + 自動
  ...
  **観測 (Linux x86_64, cpu_count=64, OpenBLAS, AVX2 + FMA, DDR4 multi-channel,
  ...
  (Phase 6 D)** として切り出し, 高 i pass を group-fused 3-phase 形に
  書き換え済み (§5.1.4)。
...
- **D (issue #79, 実装済み)**: `apply_h_kryanneal` の DRAM bandwidth 改善.
  `apply_h_kryanneal_rayon` を **group-fused 3-phase 形** (per-chunk diag +
  low-i / group-fused 高 i / per-chunk 残り高 i) に書き換え, 連続 fused_k 個
  の高 i axes (mask ≥ chunk_size) の partner 参照を 2^fused_k 個の連続 chunk
  から成る group (L2 resident) 内で完結させて DRAM v traffic を削減
  (§5.1.4). 初期版は A (group fusion) のみ. issue #79 で sweep 候補に挙がった
  B (SIMD i ≥ 3 拡張) / C (prefetch) / D (streaming store) は acceptance 達成
  状況次第で別 phase に切り出す.
