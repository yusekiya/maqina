...
API の網羅的リファレンスは [`python/kinema/*.pyi`](../python/kinema/)
スタブを参照する。
...
```bash
uv add 'git+https://github.com/yusekiya/kinema'
```
...
```python
import numpy as np
from kinema import IsingProblem, Schedule, QuantumAnnealer, Observable
from kinema.initial_states import uniform_superposition
...
`kinema` の thread pool は **プロセス起動時の環境変数で確定し,
以降縮小できない** (rayon の global pool / BLAS の thread pool いずれも
最初の op で初期化される) ため, **`kinema` / `numpy` を import する
...
迷ったら **全部同じ値で揃える** のが安全。`kinema.set_blas_threads(n)` /
`kinema.set_blas_threads_auto()` は import 後に動的に active BLAS
thread 数を絞る補助関数だが, **pool size 自体は縮まない**
(sleeping thread の stack / kernel resource は残る) ので per-process
隔離の主役にはならない。

`kinema.set_blas_threads_auto()` は issue #116 (2026-05-21) で導入された
...
`initializer` で子プロセス内 (= `kinema` import 前) に env を set する。
`fork` 文脈だと親で既に numpy / kinema を import 済みの場合に env が
...
def _init_worker():
    # 子プロセスで kinema / numpy import より前に実行される.
    os.environ["RAYON_NUM_THREADS"] = str(THREADS_PER_WORKER)
    os.environ["OPENBLAS_NUM_THREADS"] = str(THREADS_PER_WORKER)
    os.environ["OMP_NUM_THREADS"] = str(THREADS_PER_WORKER)

def worker(job):
    import kinema  # この時点で初めて import (env が効く)
    # ... 計算 ...
    return result
...
- `kinema.set_blas_threads(n)` / `kinema.set_blas_threads_auto()` の
  用途と限界: docstring 参照 (import 後に動的に active BLAS thread 数を絞る
  補助; pool size は変えない)

## 次に読むもの

- 公開 API リファレンス: [`python/kinema/*.pyi`](../python/kinema/)
