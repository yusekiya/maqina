...
def _try_import_rust() -> ModuleType | None:
    ...
    ``from maqina import _rust`` 形式だと未解決 import エラーになる.
...
def _adaptive_dispatch_richardson_estimate_chebyshev_xyz(
    ...
    if rust_mod is None:
        raise NotImplementedError(
            ...
            "extension (maqina._rust). Install with 'uv run maturin develop "
    ...
    **同じ PI controller 構造** を維持したまま, 各 stage の短時間プロパゲータ
    ...
    if rust_mod is None:
        ...
        raise NotImplementedError(
            ...
            "extension (maqina._rust). Build with 'uv run maturin develop "
