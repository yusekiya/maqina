...
問題ハミルトニアン H_p の **dynamic range が狭いケース** (SK random) と
**広いケース** (SK + 一部の計算基底に大きなペナルティを加算) を比較
(N=18, T=10^4).

| narrow dynamic range | wide dynamic range |
|---|---|
| ![narrow](docs/figures/0.8.0_pareto_non_stiff.png) | ![wide](docs/figures/0.8.0_pareto_stiff.png) |

注: kryanneal は rayon で全 CPU コアを使う一方, QuTiP は sparse 経路 (大規模
N で必須) の制約上ほぼシングルスレッド相当. 図中の wall time の絶対値は
このスレッド非対称性込みの「各ツールの典型的な使い方での比較」と読む.
