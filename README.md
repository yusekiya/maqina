...
長時間 (long-T) のアニーリングや stiff な目的関数 (ペナルティ法による
dynamic range 拡大) で kryanneal の matrix-free + adaptive Richardson 経路は
QuTiP `sesolve` (sparse) に対し Pareto 上の同等以上を握る。下記は **N=18,
T=10^4** での fidelity-vs-runtime 散布図 (両軸 log, 左下が優勢領域).
参照解は QuTiP Adams で許容誤差 sweep + BDF 一致確認まで取った高精度解.

| non-stiff (SK random) | stiff (10% basis × penalty=100) |
|---|---|
| ![non-stiff](docs/figures/0.8.0_pareto_non_stiff.png) | ![stiff](docs/figures/0.8.0_pareto_stiff.png) |

再生成手順は [`benchmarks/README.md`](benchmarks/README.md) の "README figure
pipeline" 節を参照 (4 step: 問題生成 → 参照解計算 → bench sweep → 描画).
