...
A **(Ma)gnus-based (Q)uantum (I)sing (N)umerical (A)nnealer** (名前はラテン語 `machina` に由来): 横磁場イジングモデル (TFIM) の量子ダイナミクスを matrix-free に計算するシミュレータ。
...
- API リファレンス: `python/maqina/*.pyi` (per-module PEP 484 stub, full docstring 付き。`tools/gen_api_stubs.py` で自動生成)

## Development

repo を clone してローカル開発する手順。`maturin develop` で Rust 拡張
`maqina._rust` を `python/maqina/` 配下に直接配置する。
