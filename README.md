...
# pip を使う場合:
pip install 'git+https://github.com/yusekiya/kinema'
```
...
- API リファレンス: `python/kinema/*.pyi` (per-module PEP 484 stub, full docstring 付き。`tools/gen_api_stubs.py` で自動生成)

## Development

repo を clone してローカル開発する手順。`maturin develop` で Rust 拡張
`kinema._rust` を `python/kinema/` 配下に直接配置する。
