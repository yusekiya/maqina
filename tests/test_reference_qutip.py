...
QuTiP との比較で maqina だけが有利になる, という偏ったベンチを避ける
...
* maqina: bit 0 = LSB, ``x = Σ_i b_i · 2^i``. X_i は bit i を flip.
* QuTiP: ``qutip.tensor([A_0, A_1, ..., A_{n-1}])`` は ``np.kron`` 順に積み,
  最初の引数 ``A_0`` が **MSB 側** (最も "遅い" qubit) に対応する.
* したがって maqina の bit i に X を作用させるには
  ``qutip.tensor([I, ..., X, ..., I])`` の **位置 ``n-1-i``** に sx を入れる.
  これにより maqina の bit-flip pattern (``k ^ (1 << i)``) と数値一致する.
"""
...
def _build_qutip_hamiltonian(h_x: np.ndarray, h_p_diag: np.ndarray, T: float) -> list:
    ...
    sparse (CSR backing) 構築する. maqina の LSB bit 規約と QuTiP の
    ...
    for i in range(n):
        ops = [si] * n
        ops[n - 1 - i] = sx  # LSB i in maqina → position n-1-i in QuTiP MSB-first
