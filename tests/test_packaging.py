...
``maturin develop`` で構築した ``kinema._rust`` がロードでき, パッケージ
...
def test_import_kinema() -> None:
    """``import kinema`` がエラー無く成功し, 公開シンボルが見える."""
    import kinema

    # __init__.py の __all__ に列挙したシンボルは module attribute として
    # 露出している.
    assert hasattr(kinema, "set_blas_threads")
    assert hasattr(kinema, "available_blas_threads")
    assert hasattr(kinema, "set_blas_threads_auto")
    assert callable(kinema.set_blas_threads)
    assert callable(kinema.available_blas_threads)
    assert callable(kinema.set_blas_threads_auto)


def test_rust_extension_loads() -> None:
    """Rust 拡張モジュール ``kinema._rust`` がロード可能.
...
def test_available_blas_threads_returns_positive_int() -> None:
    ...
    from kinema import available_blas_threads
...
def test_recommended_blas_threads_returns_positive_int() -> None:
    ...
    from kinema import _recommended_blas_threads
...
def test_read_env_cap_priority(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from kinema import _read_env_cap
...
def test_set_blas_threads_auto_returns_applied_value() -> None:
    ...
    """
    from kinema import _recommended_blas_threads, set_blas_threads_auto
...
def test_target_feature_flags_are_bool() -> None:
    ...
    ``target-cpu=native`` の効きの実値確認は ``kinema.show_config()`` の
    手動実行で行う方針.
    """
    _rust = pytest.importorskip("kinema._rust")
...
def test_show_config_runs(capsys: pytest.CaptureFixture[str]) -> None:
    """``kinema.show_config()`` が呼び出し可能で構成情報を stdout に出力する.
    ...
    assert "kinema build configuration" in captured.out
