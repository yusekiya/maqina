...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "maqina observables / save_tlist overhead benchmark (Phase 5, issue #47)"
        )
    )
    parser.add_argument(
        "--n-values",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "if specified, call maqina.set_blas_threads(N) at startup. "
...
def _gather_machine_info(args: argparse.Namespace) -> dict[str, str]:
    ...
    return {
        ...
        "maqina_blas_feature": str(has_blas),
