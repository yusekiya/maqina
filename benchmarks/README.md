...
| `bench_simd_scaling.py` | `apply_h` per-pass time の SIMD ON/OFF 比較. `measure` subcommand を異なる build で 2 回回し (`simd-on` / `simd-off` label), `compare` subcommand で speedup table を MD/CSV に統合. i=0,1,2 集中時 (h_x=[1,1,1,0,...,0]) と全 i (h_x=all-ones) の 2 モードを取る | Phase 6 C2 (issue #63) |
...
| Krylov fixed (cfm4) | 0.8.0 流用 (controller 非依存 + 参照解不変) | **再計算** (新 BDF 参照に統一) | stiff のみ ✅ ※ |
| QuTiP | **再計算** (状態保存目的) | **再計算** (新 BDF 参照 + 状態保存) | ✅ |

> 固定 cfm4 の ψ 保存方針 (#159): 「再計算しないものをわざわざ計算してまで保存はしない /
> 再計算する以上はそのとき保存する」。
> - **narrow cfm4 は 0.8.0 を流用** (固定 cfm4 は 0.14.0 の adaptive controller 変更の影響を
>   受けず, narrow 参照解も不変なので infidelity 有効; 同一マシン・同一アルゴリズムで
>   wall_sec も比較可能)。再計算しないので ψ 保存対象外 (伝播器実装が変われば次回再計算する
>   ときに保存すればよい)。
> - **stiff cfm4 は新 BDF 参照のため再計算する**ので, そのとき ψ も保存する。
>
> ※ 0.14.0 の実行は本スクリプト更新 (cfm4 ψ 保存化) より前に開始したため, 0.14.0 の
> 成果物には stiff cfm4 ψ が含まれない (CSV 行のみ)。次回以降の run では保存される。
...
- maqina (version 依存): `benchmarks/results/0.14.0/bench_<scenario>.csv`
  + `states/` (22 cell = adaptive 18 + stiff cfm4 4; ※ 0.14.0 実行はスクリプト更新前開始のため
  stiff cfm4 ψ は欠落, 次回以降保存)
- QuTiP (version 非依存): `benchmarks/results/qutip/bench_<scenario>.csv` + `states/` (8 cell)

保存 ψ 合計 30 cell ≒ 115 MiB。所要は QuTiP だけで約 65h (stiff ~48h + non-stiff ~17h,
既存実測ベース) 追加 (Phase 2 並列実行で実時間 ~48h)。
