...
各 cell (1 つの atol / dt / tol) 完了ごとに **CSV を atomic に書き直す**
(tmp + os.replace) ため, 途中で kill / 電源断 / SSH 切断が起きても **既に
完了した cell の結果は保持** される. 起動時に既存の CSV を読んでロード済の
``(solver, variant, knob_value)`` を skip するため, 同じ引数で再起動するだけで
残りの cell から再開できる.

## 0.8.0 既収集 CSV との結合運用
...
def _run_kinema_adaptive_chebyshev(
    ...
    Krylov 版と同じ PI controller 構造で短時間プロパゲータだけが Chebyshev
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "0.8.0 既収集の atol=1e-9 cell は飽和 (infidelity≈0) のため default から除外.",
    )
    parser.add_argument(
        "--cfm4-dts",
        type=_parse_floats,
