...
- **timestamp_utc**: `2026-05-26T03:42:43+00:00` ~ `2026-05-26T04:22:25+00:00`
  (約 40 min)
...
→ **non-stiff Pareto frontier は全 4 Chebyshev cell + QuTiP の最 tight cell
...
完全に単調。0.11.0 までの auto-coupling default (`propagator_tol = atol·1e-3`)
で観測されていた "1e-4 で floor → 1e-5 で 1.78e-6 悪化" の非単調性は完全に解消。
...
同 atol で **4.0-4.4× 高速**。Lanczos の V 行列 (dim × m_max ≈ 96 MB for
...
で達成**。QuTiP の中間 tol (`1e-5`, `1e-7`) は非単調 / 失敗で実用上 Pareto
にいないので, "QuTiP の精度帯にいる" 比較は最 tight cell のみ。
