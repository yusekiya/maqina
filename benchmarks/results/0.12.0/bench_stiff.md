...
| ✓ | **kinema 0.12.0** | **chebyshev_adaptive** | **atol=1e-6** | **5635.57** | **3.338e-10** ⚠️ | **59506** |
...
|   | **kinema 0.12.0** | **chebyshev_adaptive** | **atol=1e-7** | 9210.83 | 4.378e-09 ⚠️ (reference floor 反転) | 111402 |
...
`atol=1e-7` で **逆に 13× 悪化**。これは kinema 側の数値破綻ではなく
**reference solution (QuTiP sesolve Adams) の precision floor (~1e-10) に
kinema 側が到達した結果**:

1. atol↓ で kinema の dt がさらに小さくなる
2. n_steps 増 (59506 → 111402, 1.87×)
3. kinema 内部の round-off accumulation + reference noise の両方が乗る
4. reference precision floor (~1e-10) を kinema infidelity が下回ろうと
