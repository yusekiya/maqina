"""maqina 公開 API の per-module ``.pyi`` スタブを生成する.

各公開モジュール (``python/maqina/<name>.py``) の AST を走査し,
``python/maqina/<name>.pyi`` に以下を出力する:
...
``python/maqina`` 配下または本スクリプト自体の変更時に自動実行する.
差分が出ると pre-commit が失敗するので
``git add python/maqina/*.pyi`` して再 commit する.

本スクリプトは cv_ising の ``tools/gen_api_stubs.py`` を maqina の
...
PKG_DIR = REPO_ROOT / "python" / "maqina"
...
def is_public(name: str, all_set: set[str] | None) -> bool:
    ...
    (``from maqina.krylov import _cfm4_step_with_m2_estimate`` 等) で
...
def _rewrite_import_as_reexport(node: ast.ImportFrom) -> ast.ImportFrom:
    ...
    ``from maqina.krylov import DENSE_GROUNDSTATE_THRESHOLD`` のように
