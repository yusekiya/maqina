---
name: test-runner
description: kinema のテスト・lint・build (`uv run pytest` / `cargo test` BLAS on/off / `uv run pre-commit run --all-files` / `uv run maturin develop --uv`) を走らせるときに参照する一次資料。コマンド・マーカ・テスト役割分担・CI フロー・既知の落とし穴を集約する。kinema 固有の `--uv` フラグや BLAS feature 切替などの手順をここに置く。`/test-runner` で発火可能。`test-runner` subagent (`.claude/agents/test-runner.md`) もここを `Read` してから実行する。
---

# test-runner skill

`kinema` のテスト実行手順を集約した一次資料 (skill body + 補助コマンド表).
...
- **user-level `/test` skill** (汎用): どのプロジェクトでも `docs/testing.md` を読む実装. kinema では `docs/testing.md` が本 skill へのポインタなので、結局ここに辿り着く.
...
| 型チェック | `uv run ty check python/kinema` |
...
Rust 拡張 `kinema._rust` は `maturin develop` で `python/kinema/` 配下に
...
rayon 経路 (`apply_h_kinema_rayon` / `apply_single_mode_axis_i_rayon`,
...
- `apply_h_kinema` の matvec が dense 構築版と一致
...
| `test_matvec.py`            | Rust `apply_h_kinema` が dense 構築版と一致 |
...
| `ty-check` | `uv run ty check python/kinema` (`.py`/`.pyi` 変更時, project mode) |
| `cargo-fmt` | `cargo fmt --check` (Rust ソース変更時) |
| `cargo-clippy` | `cargo clippy --all-targets -- -D warnings` (Rust ソース変更時) |
| `gen-api-stubs` | `uv run python tools/gen_api_stubs.py` (`python/kinema/**/*.py` または `tools/gen_api_stubs.py` 変更時) |
...
節). pre-commit が落ちたら `git add python/kinema/*.pyi` で取り込み再 commit.
...
- `.claude/solve-overrides.md`: `/solve` skill の kinema 固有 delta.
